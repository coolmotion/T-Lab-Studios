...
function displayProducts(products) {
    ...
    products.forEach(product => {
        ...
        const productNameforwa = product.name; // Assuming the product name is stored in productName field
        const whatsappMessage = `Hello, I'm interested in ${productNameforwa} . Can you provide more details?`;
        const whatsappLink = `https://wa.me/+94777836315/?text=${encodeURIComponent(whatsappMessage)}`;

        const preOrderLink = document.createElement('a');
        preOrderLink.href = whatsappLink;
        preOrderLink.className = 'bg-orange-500 text-white rounded-full px-4 py-2 mt-4';
        preOrderLink.textContent = 'Pre Order';
